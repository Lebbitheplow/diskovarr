...
- **Guided DUMB setup (Admin → Setup)** — a five-step wizard that installs and wires a [DUMB Traktless](https://github.com/Lebbitheplow/DUMB-traktless) stack: detects a running DUMB (or writes the compose file and starts the container when Docker is reachable), validates your AllDebrid key (or Real-Debrid, with a fewer-results warning; RD is served by Zurg instead of Decypharr), lets you pick extra apps (Zilean, Sonarr, Radarr, Prowlarr, Seerr, Tautulli, Riven UI), then has DUMB install everything, feed Riven your TMDB key (TVDB uses Riven's bundled key) and Plex, provision the Riven ↔ Diskovarr request bridge, and push the new URLs/API keys into Connections. Finally it adds the symlink library folders to your Plex movie/TV libraries with debrid-friendly scan settings (no preview thumbnails, TV intro/credits detection off, partial scans on, auto-empty-trash off) — or prints exact instructions if you'd rather do it by hand. Needs the Traktless fork's `/diskovarr/provision` API.
