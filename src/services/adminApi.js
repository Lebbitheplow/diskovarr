...
export const adminAutomation = {
  ...
  lookup: (tmdbId, mediaType) => adminApi.get('/automation/lookup', { params: { tmdbId, mediaType } }),
  getLimits: () => adminApi.get('/automation/limits'),
  setLimits: (limits) => adminApi.post('/automation/limits', limits),
  getExclusions: () => adminApi.get('/automation/exclusions'),
  setExclusions: (exclusions) => adminApi.post('/automation/exclusions', { exclusions }, { timeout: 60000 }),
  quickSync: () => adminApi.post('/automation/quick-sync', {}, { timeout: 300000 }),
  getHubLayout: () => adminApi.get('/automation/hubs', { timeout: 60000 }),
  saveHubLayout: (sectionId, items) => adminApi.put(`/automation/hubs/${sectionId}`, { items }, { timeout: 120000 }),
  applyHubLayout: () => adminApi.post('/automation/hubs/apply', {}, { timeout: 180000 }),
