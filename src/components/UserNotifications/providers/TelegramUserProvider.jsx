  ...
  return (
    <section className="admin-section">
      <div className="admin-section-header">
        <h2 className="section-title">{t('Telegram')}</h2>
      </div>
      <p className="section-desc" style={{ marginBottom: 16 }}>{t('Get notifications via Telegram. Your admin must have configured a Telegram bot.')}</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div>
          <label className="conn-label">{t('Chat ID')}</label>
          <input type="text" className="conn-input" placeholder={t('Chat ID')} value={chatId} onChange={(e) => setChatId(e.target.value)} />
        </div>
        <div>
          <label className="conn-label">{t('Thread ID')} <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(optional, forum topics)</span></label>
          <input type="text" className="conn-input" placeholder={t('Thread ID')} value={threadId} onChange={(e) => setThreadId(e.target.value)} />
        </div>
        ...
        <div className="toggle-row">
          ...
          <span className="toggle-label">{t('Enable Telegram notifications')}</span>
        </div>
