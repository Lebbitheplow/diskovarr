...
export default function DiscordUserProvider({ settings, onToast, onSave }) {
  ...
  return (
    <section className="admin-section">
      ...
      <p className="section-desc" style={{ marginBottom: 16 }}>{t('Receive direct message notifications from the Diskovarr bot.')}</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div>
          <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', margin: '0 0 6px' }}>
            <strong style={{ color: 'var(--text-primary)' }}>{t('Desktop:')}</strong> Enable Developer Mode (Settings {'>'} Advanced), right-click your avatar {'>'} Copy User ID.
          </p>
          <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', margin: '0 0 10px' }}>
            <strong style={{ color: 'var(--text-primary)' }}>{t('Mobile:')}</strong> Tap your profile in a message, tap the &#8942; {'>'} Copy User ID.
          </p>
        ...
        <div className="toggle-row">
          ...
          <span className="toggle-label">{t('Enable Discord notifications')}</span>
        </div>
        {settings?.discord_invite_link && (
          <div style={{ background: 'rgba(255,165,0,0.08)', border: '1px solid rgba(255,165,0,0.25)', borderRadius: 8, padding: '10px 14px', fontSize: '0.82rem', color: 'var(--text-secondary)' }}>
            ...
            <a href={settings.discord_invite_link} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', marginTop: 8, padding: '6px 14px', background: 'var(--accent)', color: '#000', borderRadius: 6, fontWeight: 600, textDecoration: 'none', fontSize: '0.82rem' }}>{t('Join Server')}</a>
          </div>
        )}
