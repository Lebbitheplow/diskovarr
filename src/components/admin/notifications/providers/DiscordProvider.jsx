    ...
    <section className="admin-section">
      <div className="admin-section-header">
        <h2 className="section-title">
          {t('Discord Agent')}
          <button type="button" className="agent-info-btn" onClick={() => onOpenAgentInfo?.('discord')} title={t('How to configure')} style={{ marginLeft: 8, background: 'none', border: 'none', cursor: 'pointer', fontSize: 'inherit', color: 'inherit', padding: 0, verticalAlign: 'middle' }}>&#9432;</button>
        </h2>
      ...
      <p className="section-desc" style={{ marginBottom: 16 }}>
        {t('Send Discord notifications via webhook or direct messages via bot. Both can be enabled independently.')}
      </p>
      ...
      <div className="toggle-row" style={{ marginBottom: 12 }}>
        ...
        <span className="toggle-label">{t('Webhook')}</span>
      </div>
      {webhookEnabled && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20, padding: 14, background: 'var(--bg-elevated)', borderRadius: 8, border: '1px solid var(--border)' }}>
          <div>
            ...
            <input type="url" className="conn-input" placeholder={t('https://discord.com/api/webhooks/...')} value={webhookUrl} onChange={(e) => setWebhookUrl(e.target.value)} />
          </div>
          <div>
            <label className="conn-label">{t('Notification Role ID')} <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(optional)</span></label>
            <input type="text" className="conn-input" placeholder={t('Role ID')} value={notificationRoleId} onChange={(e) => setNotificationRoleId(e.target.value)} />
          </div>
          <Checkbox checked={enableMentions} onChange={(e) => setEnableMentions(e.target.checked)}>{t('Enable @role mentions')}</Checkbox>
          <div>
          ...
          <Checkbox checked={webhookEmbedPoster} onChange={(e) => setWebhookEmbedPoster(e.target.checked)}>{t('Embed poster image')}</Checkbox>
      ...
      <div className="toggle-row" style={{ marginBottom: 12 }}>
        ...
        <span className="toggle-label">{t('Bot Token')}</span>
      </div>
      {botEnabled && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20, padding: 14, background: 'var(--bg-elevated)', borderRadius: 8, border: '1px solid var(--border)' }}>
          <div>
            <label className="conn-label">{t('Bot Token')}</label>
            <input type="password" className="conn-input" placeholder={t('Bot token from Discord Developer Portal')} value={botToken} onChange={(e) => setBotToken(e.target.value)} autoComplete="off" />
          </div>
          <div>
            <label className="conn-label">{t('Bot Username')}</label>
            <input type="text" className="conn-input" placeholder={t('Diskovarr')} value={botUsername} onChange={(e) => setBotUsername(e.target.value)} />
          </div>
          <div>
            <label className="conn-label">{t('Bot Avatar')} <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(optional)</span></label>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 4 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <label htmlFor="discord-avatar-file" className="btn-admin btn-sm" style={{ cursor: 'pointer', margin: 0 }}>{t('Choose Image')}</label>
                  ...
                  <button type="button" className="btn-admin btn-sm btn-danger" style={{ display: avatarPreview ? 'inline-block' : 'none' }} onClick={() => { setAvatarPreview(null); setAvatarFileData(null); setAvatarRemoving(true) }}>{t('Remove')}</button>
                </div>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{t('PNG/JPG/GIF · 128×128 recommended · max 2 MB')}</span>
              </div>
            </div>
            <div style={{ marginTop: 8 }}>
              <label className="conn-label" style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>{t('Or use a URL')}</label>
              <input type="text" className="conn-input" placeholder={t('https://example.com/avatar.png')} value={botAvatarUrl} onChange={(e) => setBotAvatarUrl(e.target.value)} style={{ marginTop: 2 }} />
            </div>
          </div>
          <Checkbox checked={botEmbedPoster} onChange={(e) => setBotEmbedPoster(e.target.checked)}>{t('Embed poster image')}</Checkbox>
          <div>
            <p style={{ fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-secondary)', margin: '0 0 8px' }}>{t('Notification Types')}</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <div>
              <label className="conn-label" style={{ marginBottom: 4 }}>{t('Your Discord User ID')} <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(for test DM)</span></label>
              <input type="text" className="conn-input" style={{ maxWidth: 240 }} placeholder={t('Discord user ID')} value={discordTestUserId} onChange={(e) => setDiscordTestUserId(e.target.value)} />
            </div>
      ...
      <div style={{ marginTop: 16, paddingTop: 14, borderTop: '1px solid var(--border)' }}>
        <label className="conn-label">{t('Discord Server Invite Link')} <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(optional)</span></label>
        <input type="url" className="conn-input" placeholder={t('https://discord.gg/...')} value={inviteLink} onChange={(e) => setInviteLink(e.target.value)} />
        <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', marginTop: 6 }}>{t('Use a non-expiring invite so users can join for bot DMs.')}</p>
      </div>
    </section>
  )
}
