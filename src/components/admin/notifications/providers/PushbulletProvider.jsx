    ...
    <section className="admin-section">
      <div className="admin-section-header">
        <h2 className="section-title">
          {t('Pushbullet')}
          <button type="button" className="agent-info-btn" onClick={() => onOpenAgentInfo?.('pushbullet')} title={t('How to configure')} style={{ marginLeft: 8, background: 'none', border: 'none', cursor: 'pointer', fontSize: 'inherit', color: 'inherit', padding: 0, verticalAlign: 'middle' }}>&#9432;</button>
        </h2>
      ...
      <p className="section-desc" style={{ marginBottom: 16 }}>{t('Send push notifications via Pushbullet. Users can set their own tokens in profile settings.')}</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
        <div>
          <label className="conn-label">{t('Access Token')}</label>
          <input type="password" className="conn-input" placeholder={t('o.YourAccessHere')} value={accessToken} onChange={(e) => setAccessToken(e.target.value)} autoComplete="off" />
          <span className="conn-hint">{t('Get from')} <a href="https://www.pushbullet.com/#settings" target="_blank" rel="noopener noreferrer">{t('Pushbullet Settings')}</a></span>
        </div>
        <div>
          <label className="conn-label">{t('Channel Tag')} <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(optional)</span></label>
          <input type="text" className="conn-input" placeholder={t('diskovarr')} value={channelTag} onChange={(e) => setChannelTag(e.target.value)} />
        </div>
        <div>
