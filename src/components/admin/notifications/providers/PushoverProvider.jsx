    ...
    <section className="admin-section">
      <div className="admin-section-header">
        <h2 className="section-title">
          {t('Pushover Agent')}
          <button type="button" className="agent-info-btn" onClick={() => onOpenAgentInfo?.('pushover')} title={t('How to configure')} style={{ marginLeft: 8, background: 'none', border: 'none', cursor: 'pointer', fontSize: 'inherit', color: 'inherit', padding: 0, verticalAlign: 'middle' }}>&#9432;</button>
        </h2>
      ...
      <p className="section-desc" style={{ marginBottom: 20 }}>{t('Send push notifications via Pushover to your devices.')}</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
        <div>
          <label className="conn-label">{t('Application API Token')}</label>
          <input type="text" className="conn-input" placeholder={t('App token from pushover.net')} value={appToken} onChange={(e) => setAppToken(e.target.value)} />
          <span className="conn-hint"><a href="https://pushover.net/api#registration" target="_blank" rel="noopener noreferrer">{t('Register an application')}</a> {t('for Diskovarr')}</span>
        </div>
        <div>
          <label className="conn-label">{t('User or Group Key')}</label>
          <input type="text" className="conn-input" placeholder={t('User/group key')} value={userKey} onChange={(e) => setUserKey(e.target.value)} />
          <span className="conn-hint">{t('Your 30-character')} <a href="https://pushover.net/api#identifiers" target="_blank" rel="noopener noreferrer">{t('User or Group ID')}</a></span>
        </div>
        <Checkbox checked={embedPoster} onChange={(e) => setEmbedPoster(e.target.checked)}>{t('Embed poster image')}</Checkbox>
        <div>
          <label className="conn-label">{t('Notification Sound')}</label>
