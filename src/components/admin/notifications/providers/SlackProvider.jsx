    ...
    <section className="admin-section">
      <div className="admin-section-header">
        <h2 className="section-title">
          {t('Slack')}
          <button type="button" className="agent-info-btn" onClick={() => onOpenAgentInfo?.('slack')} title={t('How to configure')} style={{ marginLeft: 8, background: 'none', border: 'none', cursor: 'pointer', fontSize: 'inherit', color: 'inherit', padding: 0, verticalAlign: 'middle' }}>&#9432;</button>
        </h2>
      ...
      <p className="section-desc" style={{ marginBottom: 16 }}>{t('Send Slack notifications via incoming webhook using Block Kit formatting.')}</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
        <div>
          <label className="conn-label">{t('Webhook URL')}</label>
          <input type="url" className="conn-input" placeholder={t('https://hooks.slack.com/services/T00/B00/XXX')} value={webhookUrl} onChange={(e) => setWebhookUrl(e.target.value)} />
          <span className="conn-hint"><a href="https://api.slack.com/messaging/webhooks" target="_blank" rel="noopener noreferrer">{t('Create an incoming webhook')}</a> {t('in Slack')}</span>
        </div>
        <div>
