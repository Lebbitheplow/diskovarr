...
- **Auto-request limits.** Rolling-window quotas for monitored lists — the per-service-user limits Agregarr relied on, applied per list: N movies per X days and N seasons per Y days (`autorequest_limits` setting, default 20/7 and 20/3, `GET/POST /admin/automation/limits`) with per-list overrides (`limit_override`, `movie_limit`, `movie_window_days`, `season_limit`, `season_window_days`). Usage is counted from each list's own requests (`discover_requests.origin_list_id`, back-filled for existing auto-requests; denied ones don't count); a show whose seasons wouldn't fit waits for a later sync instead of being partially requested, and the sync status reports how many items are waiting on the limit. The queue shows which list made each auto-request.
