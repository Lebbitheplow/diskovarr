...
// Stage colour: 6% of the accent folded into the warm near-black base. Must
// match ThemeContext.stageColor and the pre-paint script in index.html.
function stageColor(hex) {
  ...
  return { hex: '#' + c.map(n => n.toString(16).padStart(2, '0')).join(''), rgb: c.join(', ') }
}
...
app.get('/manifest.json', (req, res) => {
  ...
  res.json({
    ...
    start_url: '/',
    display: 'standalone',
    background_color: stageColor(color).hex,
    theme_color: color,
    icons: [
